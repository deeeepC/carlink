...
int comms_control_handler(ControlPacket_t *req, uint8_t *resp) {
  ...
  switch (req->request) {
    ...
    case 0xde:
      if ((req->param1 < CARLINK_BUS_CNT) && is_speed_valid(req->param2, speeds, sizeof(speeds)/sizeof(speeds[0]))) {
    ...
    case 0xf1:
      if (req->param1 == 0xFFFFU) {
        ...
        can_clear(&can_rx_q);
      } else if (req->param1 < CARLINK_BUS_CNT) {
    ...
    case 0xfc:
      if ((req->param1 < CARLINK_CAN_CNT) && current_board->has_canfd) {
